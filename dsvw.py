...
try:
    ...
    print(
        "[!] please install 'python-lxml' to (also) get access to XML vulnerabilities (e.g. '%s')\n"
        % (
            "apt-get install python-lxml"
            if os.name != "nt"
            else "https://pypi.python.org/pypi/lxml"
        )
    )

NAME, VERSION, GITHUB, AUTHOR, LICENSE = (
...
(
    ...
    NAME
), '<div style="position: fixed; bottom: 5px; text-align: center; width: 100%%;">Powered by <a href="%s" style="font-weight: bold; text-decoration: none; visited: blue; color: blue" target="_blank">%s</a> (v<b>%s</b>)</div>\n</body>\n</html>' % (
    GITHUB,
...
CASES = (
    (
        "Blind SQL Injection (<i>boolean</i>)",
        "?id=2",
    ...
    (
        "Cross Site Scripting (<i>reflected</i>)",
        "/?v=0.2",
    ...
    (
        "XML External Entity (<i>local</i>)",
        "/?xml=%3Croot%3E%3C%2Froot%3E",
        (
            "/?xml=%3C!DOCTYPE%20example%20%5B%3C!ENTITY%20xxe%20SYSTEM%20%22file%3A%2F%2F%2Fetc%2Fpasswd%22%3E%5D%3E%3Croot%3E%26xxe%3B%3C%2Froot%3E"
            if os.name != "nt"
            else "/?xml=%3C!DOCTYPE%20example%20%5B%3C!ENTITY%20xxe%20SYSTEM%20%22file%3A%2F%2FC%3A%2FWindows%2Fwin.ini%22%3E%5D%3E%3Croot%3E%26xxe%3B%3C%2Froot%3E"
        ),
    ...
    (
        "XML External Entity (<i>remote</i>)",
        "/?xml=%3Croot%3E%3C%2Froot%3E",
    ...
    (
        ...
        "/?path=",
        (
            "/?path=http%3A%2F%2F127.0.0.1%3A631"
            if os.name != "nt"
            else "/?path=%5C%5C127.0.0.1%5CC%24%5CWindows%5Cwin.ini"
        ),
        "http://www.bishopfox.com/blog/2015/04/vulnerable-by-design-understanding-server-side-request-forgery/",
    ),
    (
        "Blind XPath Injection (<i>boolean</i>)",
        "/?name=dian",
    ...
    (
        "Frame Injection (<i>content spoofing</i>)",
        "/?v=0.2",
    ...
    (
        ...
        "/?redir=",
        "/?redir=http%3A%2F%2Fdsvw.c1.biz",
    ...
    (
        ...
        (
            "/?domain=www.google.com%3B%20ifconfig"
            if os.name != "nt"
            else "/?domain=www.google.com%26%20ipconfig"
        ),
    ...
    (
        ...
        (
            ...
            if os.name != "nt"
            else "/?path=..%5C..%5C..%5C..%5C..%5C..%5CWindows%5Cwin.ini"
        ),
        "https://www.owasp.org/index.php/Path_Traversal",
    ),
    (
        "File Inclusion (<i>remote</i>)",
        "/?include=",
    ...
    (
        "HTTP Header Injection (<i>phishing</i>)",
        "/?charset=utf8",
    ...
    (
        "Component with Known Vulnerability (<i>pickle</i>)",
        "/?object=%s"
        % urllib.parse.quote(
            pickle.dumps(
                dict(
                    (
                        _.findtext("username"),
                        (_.findtext("name"), _.findtext("surname")),
                    )
                    for _ in xml.etree.ElementTree.fromstring(USERS_XML).findall("user")
                )
            )
        ),
        "/?object=cos%%0Asystem%%0A(S%%27%s%%27%%0AtR.%%0A\" onclick=\"alert('checking if arbitrary code can be executed remotely (true in case of delayed response)')"
        % urllib.parse.quote(
        ...
        try:
            if path == "/":
                ...
                if "id" in params:
                    ...
                    content += (
                        "<div><span>Result(s):</span></div><table><thead><th>id</th><th>username</th><th>name</th><th>surname</th></thead>%s</table>%s"
                        % (
                            "".join(
                                "<tr>%s</tr>"
                                % "".join(
                                    "<td>%s</td>" % ("-" if _ is None else _)
                                    for _ in row
                                )
                    ...
                    content = (
                        (
                            open(os.path.abspath(params["path"]), "rb")
                            if not "://" in params["path"]
                            else urllib.request.urlopen(params["path"])
                        )
                        .read()
                        .decode()
                    )
                    ...
                    content = lxml.etree.tostring(
                        lxml.etree.parse(
                            io.BytesIO(params["xml"].encode()),
                            lxml.etree.XMLParser(no_network=False),
                        ),
                        pretty_print=True,
                    ).decode()
                    ...
                    content += "<b>Surname:</b> %s%s" % (
                        found[-1].find("surname").text if found else "-",
                        HTML_POSTFIX,
                    )
                    ...
                    content += (
                        "<b>Time required</b> (to 'resize image' to %dx%d): %.6f seconds%s"
                        % (
                            int(params["size"]),
                            int(params["size"]),
                            time.time() - start,
                ...
                # Fix: Use parameterized query to prevent SQL Injection for comments
                elif "comment" in params or query == "comment=":
                    if "comment" in params:
                        cursor.execute(
                            "INSERT INTO comments (comment, time) VALUES (?, ?)",
                            (params["comment"], time.ctime()),
                        )  # Secure query
                        content += (
                            'Thank you for leaving the comment. Please click here <a href="/?comment=">here</a> to see all comments%s'
                            % HTML_POSTFIX
                        )
                    else:
                    ...
                    backup, sys.stdout, program, envs = (
                        sys.stdout,
                        io.StringIO(),
                        (
                            open(params["include"], "rb")
                            if not "://" in params["include"]
                            else urllib.request.urlopen(params["include"])
                        ).read(),
                        {
                ...
                if HTML_PREFIX in content and HTML_POSTFIX not in content:
                    content += "<div><span>Attacks:</span></div>\n<ul>%s\n</ul>\n" % (
                        "".join(
                            ...
                            % (
                                (
                                    ' class="disabled" title="module \'python-lxml\' not installed"'
                                    if (
                                        "lxml.etree" not in sys.modules
                                        and any(
                                            _ in case[0].upper()
                                            for _ in ("XML", "XPATH")
                                        )
                    ...
                    ).replace('<a href="None">vulnerable</a>|', "<b>-</b>|")
                ...
                content = "%s%s%s" % (
                    ...
                    json.dumps(
                        dict(
                            (_.findtext("username"), _.findtext("surname"))
                            for _ in xml.etree.ElementTree.fromstring(
                                USERS_XML
                            ).findall("user")
                        )
                    ),
            ...
            # Fix: Use parameterized query to prevent SQL Injection in login
            elif path == "/login":
                cursor.execute(
                ...
                content += (
                    'Welcome <b>%s</b><meta http-equiv="Set-Cookie" content="SESSIONID=%s; path=/"><meta http-equiv="refresh" content="1; url=/"/>'
                    % (
                        re.sub(r"[^\w]", "", params.get("username", "")),
                        "".join(
                    ...
                    if cursor.fetchall()
                    else 'The username and/or password is incorrect<meta http-equiv="Set-Cookie" content="SESSIONID=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT">'
                )
            ...
            self.wfile.write(
                (
                    ...
                    % (
                        ...
                        (
                            HTML_POSTFIX
                            if HTML_PREFIX in content and GITHUB not in content
                            else ""
                        ),
                    )
                ).encode()
